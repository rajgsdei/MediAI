class SharedCRUD {
    ...
    generateFormFields() {
        ...
        this.options.formFields.forEach(field => {
            ...
            // Support disabling a field via config
            if (field.disabled) {
                input.disabled = true;
            }
    ...
    saveEntity() {
        ...
        // Include disabled inputs' values because FormData(form) does not include disabled fields.
        // This ensures fields like a disabled 'status' are submitted.
        form.find(':input:disabled[name]').each(function() {
            const $el = $(this);
            const name = $el.attr('name');
            if (!name) return;
            // For checkboxes, booleans, etc., normalize value
            let val = $el.val();
            // If checkbox, ensure value reflects checked state
            if ($el.is(':checkbox')) {
                val = $el.prop('checked') ? ($el.val() || 'on') : '';
            }
